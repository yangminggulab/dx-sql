...
\end{lstlisting}
\begin{dxtips}
    今天的错误是太贪心，想少用子查询。首先应该想明白到底要链接几张表。是这样的比如最后满足我们要求的只有一个元素，这个需要count没问题但是一旦进行操作以后最开始的表一定变化了，所有一定需要一个最开始表的子查询也就是(SELECT COUNT(DISTINCT player_id) FROM Activity。除数是一个表，被除数是一个表。求除数再看需要几个表。一个表没法和自己的元素上下判断所以需要自链接弄成横着的。然后筛选条件一定要写在一起DATE_ADD(t.firstdate, INTERVAL 1 DAY);最后在筛选。
\end{dxtips}
